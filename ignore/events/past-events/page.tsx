...
async function PastEvents() {
    const events: Event[] = await fetch(API_URL + "events/past-events").then((res) => res.json());
        ...
        <div>
            <div className="bg-[var(--gray-100)] p-8 md:pl-40 md:pr-32 pb-12 md:pb-4">
                                ...
                                <div className="min-h-60 bg-white shadow-lg flex flex-col-reverse md:flex-row transform translate-y-12" key={event.title}>
                                    <div className="md:w-1/2">
                                        ...
                                        <div className="px-8 py-6">
                                            ...
                                            <p dangerouslySetInnerHTML={{ __html: event?.description }} className="line-clamp-3 text-sm mb-5 page-content prose"></p>
                                    ...
                                    <div className="md:w-1/2">
                                        {/* <img src={!event?.images?.length ? "/assets/images/events/event-placeholder.jpg" : `https://files.sringeri.net/${event?.images[0]}`} 
                                         alt={event.title} className="md:w-full max-h-72" /> */}
                                    </div>
                                </div>
                            )
                        }) : 
                ...
                <div className="flex justify-center items-center pt-11 uppercase md:pb-8 mt-14">
                    <Link href="/events/upcoming-events" className="text-white bg-[var(--events)] text-xs px-10 py-2 font-inter rounded-sm">View Upcoming Events</Link>
                </div>
            </div>
        </div>
    )

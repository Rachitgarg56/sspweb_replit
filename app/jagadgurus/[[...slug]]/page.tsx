...
const getPageData = cache(async (slugPath: string) => {
  const res = await fetch(`${API_URL + 'tp/jagadgurus' + slugPath}`);
...
export default async function Page({ params }: Props) {
  ...
  const slugPath = slug ? '/' + slug.join("/") : "";
  const pageData = await getPageData(slugPath);
  const sidebarData: SidebarItem[] = await getSidebarData('/jagadgurus');
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6 w-full py-4 px-16 bg-[#FFFFFF] max-lg:px-6 max-[462px]:px-4">
          ...
          <div dangerouslySetInnerHTML={{ __html: pageData?.content }} className='page-content xprose about'></div>
